...
The main idea behind posting this python module is for users to adapt/modify the content to meet their specific goals.


Code operation:
--------------
1. Automatically creates a directory structure for convergence calculations, 
   containing all the required files.  Basic files REQUIRED to create the directory 
   structure are POSCAR, POTCAR, KPOINTS, INCAR, VASP.qsub (or any other job submission script), 
   1-runvasploop.sh and 2-extractData.sh
    
2. Use the seprately provided shell script,1-runvasploop.sh, to run the 
   calculations by submitting jobs to your favourite cluster using 
   VASP.qsub (this will require modification depending on the cluster
   and submission module being used)

3. After the calculations are over,a second shell script, 2-extractData.sh (provided separately), 
   will extract the calculated data, and

4. The plotting utility , using Matplotlib,  will plot energies vs. the relavent parameter. 
   This plot can be further used for identifying the convergence parameters

IMPORTANT :  The current version DOES NOT connect with VASP (Vienna Ab Inito Simulation Package)
             on any cluster.
